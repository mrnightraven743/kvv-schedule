...
def safe_connect():
    """Подключение с защитой от Internal State Error"""
    if wlan.isconnected():
        return True
...
def get_static_schedule(current_h, current_m):
    ...
    for h_offset in [0, 1]:
        ...
        for minute, line, dest in trains:
            ...
            if 0 <= diff <= 90:
                # Обязательно сокращаем текст и здесь
...
def main():
    ...
    while True:
        ...
        # --- ОБНОВЛЕНИЕ В 3 ЧАСА НОЧИ ---
        if h == 3 and online and not update_done_today:
            show_status("Nightly Update...")
            if schedule_updater.update_from_github():
                # Успех -> Обратный отсчет 10 сек
                for i in range(10, 0, -1):
                ...
                update_done_today = True # Не вышло, попробуем завтра
